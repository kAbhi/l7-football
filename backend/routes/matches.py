...
def add_match():
    ...
    responses:
      ...
      400:
        description: Validation error (teams or location do not exist)
        schema:
          type: object
          properties:
            error:
              type: string
              example: One or both teams do not exist. Please check the team names.
    """
    data = request.json

    # Check if both teams exist
    team1_exists = Team.query.filter_by(name=data["team1"]).first()
    team2_exists = Team.query.filter_by(name=data["team2"]).first()
